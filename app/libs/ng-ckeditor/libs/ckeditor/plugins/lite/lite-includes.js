...
(function(b) {
    ...
    b.rangy.createCoreModule("DomUtil", [], function(N, L) {
        ...
        function aj(aw) {
            var av = ar(aw).createDocumentFragment(),
                ax;
    ...
    b.rangy.createCoreModule("DomRange", ["DomUtil"], function(S, O) {
        ...
        function a0(a6, a5, a3) {
            ...
            var a7 = aN(a6.startContainer, a6.startOffset, a5.endContainer, a5.endOffset),
                a4 = aN(a6.endContainer, a6.endOffset, a5.startContainer, a5.startOffset);
        ...
        aa.extend(S.rangePrototype, {
            ...
            compareNode: function(a5) {
                ...
                var a6 = this.comparePoint(a4, a7),
                    a3 = this.comparePoint(a4, a7 + 1);
                if (a6 < 0) {
                    return (a3 > 0) ? at : ad
                } else {
                    return (a3 > 0) ? af : U
            ...
            intersectsNode: function(a6, a3) {
                ...
                var a5 = a6.parentNode,
                    a8 = ak(a6);
                aW(a5, "NOT_FOUND_ERR");
                var a7 = aN(a5, a8, this.endContainer, this.endOffset),
                    a4 = aN(a5, a8 + 1, this.startContainer, this.startOffset);
                return a3 ? a7 <= 0 && a4 >= 0 : a7 < 0 && a4 > 0
            },
            ...
            intersection: function(a3) {
                if (this.intersectsRange(a3)) {
                    var a6 = aN(this.startContainer, this.startOffset, a3.startContainer, a3.startOffset),
                        a4 = aN(this.endContainer, this.endOffset, a3.endContainer, a3.endOffset);
        ...
        function ab(a3, a4) {
            return function() {
                ...
                var ba = this.startContainer,
                    a9 = this.startOffset,
                    a5 = this.commonAncestorContainer;
    ...
    b.rangy.createCoreModule("WrappedSelection", ["DomRange", "WrappedRange"], function(T, N) {
        ...
        var aO = aH(window, "getSelection"),
            aF = Y.isHostObject(document, "selection");
        ...
        if (ag) {
            ...
            T.isSelectionValid = function(aW) {
                var aX = aj(aW, "isSelectionValid").document,
                    aV = aX.selection;
        ...
        K.refresh = function(aW) {
            ...
            var aY = this.anchorNode,
                aZ = this.anchorOffset;
            au(this);
            if (aW) {
                var aX = aV.length;
        ...
        K.getBookmark = function(aV) {
            return {
                ...
                rangeBookmarks: this.callMethodOnEachRange("getBookmark", [aV])
            }
        };
...
(function(o, c) {
    ...
    f.prototype = {
        ...
        getCurrentUser: function() {
            var z = this.currentUser || {},
                A = (z.id === null || z.id === undefined) ? "" : String(z.id);
            return {
                name: z.name || "",
                id: A
            }
        ...
        insert: function(H) {
            ...
            var B = this.getCurrentRange(),
                C = this._isRangeInElement(B, this.element),
                A = C ? null : this.hostMethods.getHostRange(),
                G = this._startBatchChange(),
                F = Boolean(C && !C.collapsed),
                E = false;
            H = H || {};
            try {
                if (F) {
                ...
                if (C || A) {
                    ...
                    E = this._insertNodes(C, A, {
                        nodes: z,
                        text: H.text,
                        insertStubText: H.insertStubText !== false
                    })
                }
            } catch (D) {
        ...
        rejectAll: function(C) {
            if (C) {
                ...
                var E = this._insertSelector,
                    z = this._deleteSelector,
                    D, B = this,
                    A = c(this.element);
        ...
        acceptRejectChange: function(M, C) {
            var I, S, Q, N, J, W, U, R = o.dom,
                V, G = this,
                E, P, O, H = c(this.element),
                z = this._userStyles,
                B, A = this.attributes.userId,
                T = this._getIceNodeClass(p),
                D = this._getIceNodeClass(q),
                K = C && C.isAccept,
                L = C && (C.notify === false);
            if (!M) {
        ...
        _moveRangeToValidTrackingPos: function(G, C) {
            ...
            var z, A, E = -1,
                B, I = [],
                J, F, D = C ? this.hostMethods.getHostNode : e,
                K = false;
            while (!K) {
                A = G.startContainer;
                if (!A || I.indexOf(A) >= 0) {
        ...
        _cleanupTextSelection: function(A, F, z, E) {
            ...
            if (E && o.dom.isEmptyTextNode(F)) {
                var B = F.parentNode,
                    D = o.dom.getNodeIndex(F),
                    C = z ? this.hostMethods.makeHostElement : e;
                B.removeChild(F);
                D = Math.max(0, D);
                A.setStart(C(B), D);
                A.setEnd(C(B), D)
        ...
        _cleanupElementSelection: function(D, B) {
            var z, F = false,
                H = B ? this.hostMethods.getHostNode(D.startContainer) : D.startContainer,
        ...
        _addChange: function(C, E, B) {
            var D = B || this._batchChangeId || this.getNewChangeId(),
                z = this;
            if (!this._changes[D]) {
        ...
        _insertNodes: function(A, N, O) {
            var L = N || A,
                ...
                T = L.startContainer,
                F = (T && T.$) || T,
                U = N ? this.hostMethods.makeHostElement : e,
                P = X.nodes,
                W = X.insertStubText !== false,
                K = X.text,
                R, S, Y = this.env.document,
                D = false;
            var M = this._getIceNode(F, q),
                C = this._isCurrentUserIceNode(M);
            this._cleanupSelection(L, Boolean(N), true);
            if (C) {
                var G = P && P[0],
                    I = M.getAttribute(this.attributes.changeId);
                if (G) {
                    D = true;
                    L.insertNode(U(G));
                    var J = G.parentNode,
        ...
        _updateChangeTime: function(D) {
            ...
            if (C) {
                var B = (new Date()).getTime(),
                    A = c(this.element).find("[" + this.attributes.changeId + "=" + D + "]"),
                    z = this.attributes.lastTime;
                C.lastTime = B;
                A.each(function(E, F) {
                    F.setAttribute(z, B)
                })
        ...
        _deleteSelection: function(G) {
            var H = new o.Bookmark(this.env, G),
                z = o.dom.getElementsBetween(H.start, H.end),
                J = [],
                I = [],
                K = {
        ...
        _deleteRight: function(J) {
            var A = o.dom.isBlockElement(J.startContainer) && J.startContainer || o.dom.getBlockParent(J.startContainer, this.element) || null,
                L = A ? (o.dom.hasNoTextOrStubContent(A)) : false,
                H = A && o.dom.getNextContentNode(A, this.element),
                M = H ? (o.dom.hasNoTextOrStubContent(H)) : false,
                E = J.endContainer,
                D = J.endOffset,
                N, B = J.commonAncestorContainer,
                F, Q = false;
            if (L) {
                return false
            }
            if (w(B)) {
                this._addDeleteTrackingToBreak(B, {
                    range: J
                });
                return true
            }
            if (B.nodeType !== o.dom.TEXT_NODE) {
                ...
                if (B.childNodes.length > D) {
                    ...
                    if (w(K)) {
                        this._addDeleteTrackingToBreak(K, {
                            range: J
                        });
                        return true
                    }
                    ...
                    if (F) {
                        if (w(F)) {
                            this._addDeleteTrackingToBreak(F, {
                                range: J
                            });
                            return true
                        }
            ...
            if (D === E.data.length && (!o.dom.hasNoTextOrStubContent(E))) {
                ...
                if (o.dom.isChildOf(F, A) && o.dom.isStubElement(F)) {
                    return this._addDeleteTracking(F, {
                        range: J,
                        moveLeft: false,
                        merge: true
                    })
        ...
        _deleteLeft: function(H) {
            var D = o.dom.isBlockElement(H.startContainer) && H.startContainer || o.dom.getBlockParent(H.startContainer, this.element) || null,
                G = D ? o.dom.hasNoTextOrStubContent(D) : false,
                A = D && o.dom.getPrevContentNode(D, this.element),
                C = A ? o.dom.hasNoTextOrStubContent(A) : false,
                M = H.startContainer,
                B = H.startOffset,
                P = H.commonAncestorContainer,
                O, L;
            if (G) {
                return false
            }
            if (w(P)) {
                this._addDeleteTrackingToBreak(P, {
                    range: H,
                    moveLeft: true
                });
                return true
            ...
            if (B === 0 || P.nodeType !== o.dom.TEXT_NODE) {
                ...
                if (!L.isContentEditable) {
                    var z = this._addDeleteTracking(L, {
                        range: null,
                        moveLeft: true,
                        merge: true
                    });
        ...
        _addDeleteTracking: function(C, I) {
            ...
            var A = this._getIceNode(C.previousSibling, p),
                H = this._getIceNode(C.nextSibling, p);
        ...
        keyPress: function(C) {
            ...
            var z = this.getCurrentRange(),
                D, B = z && o.dom.parents(z.startContainer, "br")[0] || null;
            if (B) {
                z.moveToNextEl(B)
            }
        ...
        prepareToCut: function() {
            var C = this.getCurrentRange(),
                z = this.hostMethods.getHostRange();
        ...
        _filterChanges: function(J) {
            var D = 0,
                ...
                F, K = J || {},
                B = K.filter,
                C = K.exclude ? c.map(K.exclude, function(L) {
                    return String(L)
                }) : null,
                A = K.include ? c.map(K.include, function(L) {
                    return String(L)
                }) : null,
                E = K.verify,
                z = null;
        ...
        _loadFromDom: function() {
            ...
            var E = this.currentUser && this.currentUser.id,
                F = (this.currentUser && this.currentUser.name) || "",
                B = (new Date()).getTime(),
                D, H = new RegExp(this.stylePrefix + "-(\\d+)"),
                z = [];
        ...
        _tooltipMouseOver: function(C) {
            ...
            D = setTimeout(function() {
                var F = z.currentChangeNode(B),
                    H = F && F.getAttribute(z.attributes.changeId),
                    G = H && z.getChange(H);
                if (G) {
        ...
        _removeTrackingInRangeOld: function(B) {
            var F = this._getIceNodeClass(q),
                A = this._getIceNodeClass(p),
                z = "." + F + ",." + A,
                C = "data-ice-class",
                E = function(K) {
                    var J, I = null;
        ...
        _removeTrackingInRange: function(F) {
            var D = this._getIceNodeClass(q),
                z = this._getIceNodeClass(p),
                E = "." + D + ",." + z,
                G = this._savedNodesMap,
                H = "data-ice-class",
                A = Date.now() % 1000000,
                B = function(O) {
                    var L, N, K = null;
        ...
        getAdjacentChangeId: function(B, C) {
            var A = C ? o.dom.getNextNode(B) : o.dom.getPrevNode(B),
                z, D = null;
